    ...
    <div class="min-h-full">
      ...
      <main class="bg-gray-100">
        <div class="mx-auto max-w-7xl py-6 sm:px-6 lg:px-8">
          ...
          <div v-if="blogs.loading" class="flex justify-center">
          ...
          <div v-else>
            <div class="grid grid-cols-1 gap-5 sm:grid-cols-2 md:grid-cols-3">
                <div v-for="(blog, ind) in blogs.data" :key="blog.id" class="flex flex-col py-4 px-6 shadow-md bg-white hover:bg-gray-50 h[470px] opacity-0 animate-fade-in-down" :style="{animationDelay: `${ind * 0.1}s`}">
                  <img :src="blog.image_url" alt="img" class="w-full h-48 object-cover"/>
                  <h4 class="mt-4 text-lg font-bold">{{blog.title}}</h4>
                  <div v-html="blog.description" class="overflow-hidden flex-1"></div>

                  <div class="flex justify-between items-center mt-3">
                    <router-link :to="{name: 'ABlogView', params: {id: blog.id} }" class="flex py-2 px-4 border border-transparent text-sm rounded-md text-white bg-gray-900 hover:bg-gray-800 focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
...
// Get post loading state, which only changes when we fetch project from backend
...
store.commit("notify", {
 ...
 function deleteBlog(blog) {
    if (confirm(`Are you sure you want to delete this blog post? Operation can't be undone!!`)
    ) 
    {
